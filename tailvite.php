...
if ($argc < 2 || in_array($argv[1], array('--help', '-help', '-h', '-?'))) {
  tailvite_drush_help('drush:tailvite');
  $command = tailvite_drush_command()['tailvite'];
 ...
 */
function tailvite_drush_command() {
  $items = array();
 ...
 */
function drush_tailvite($human_readable_name = NULL) {

  // If no $human_readable_name provided, abort.
  if (!$human_readable_name) {
    print(dt('Theme name missing. See help using \'drush help tailvite\'.'));
    return;
  }
  ...
  $description = (drush_get_option('description')) ? trim(drush_get_option('description')) : 'Theme based on <a href="http://tailvite.info">tailvite</a>.';
  ...
  $status = drush_tailvite_create($human_readable_name, $machine_name, $description, $theme_path);
 ...
 */
function drush_tailvite_create($human_readable_name, $machine_name, $description, $theme_path_passed) {
  ...
  if (!empty($theme_path_passed)) {
    if ($theme_path_passed === 'none') {
      ...
      $theme_dir_status = _tailvite_validate_path($theme_dir . DIRECTORY_SEPARATOR . $theme_path_passed);
      if ($theme_dir_status !== TRUE) {
        return _tailvite_notify_fail('', 'Failed on Phase: Validate theme dir is writeable.');
      }
    }
  }
  else {
  ...
  $theme_path_status = _tailvite_validate_path($theme_path);
  if ($theme_path_status !== TRUE) {
    return _tailvite_notify_fail('', 'Failed on Phase: Validate theme path is writeable.');
  }

  // Phase: Verify there are not existing contents in the destination directory.
  $theme_path_empty_status = _tailvite_validate_path_is_empty($theme_path);
  if ($theme_path_empty_status !== TRUE) {
    return _tailvite_notify_fail('', 'Failed on Phase: Verify there are not existing contents in the destination directory.  Please either delete the contents or pick a different path by changing the machine name or using the \'--path\' option.  Use \'drush help tailvite\' for more information.');
  }

  // Phase: Make directories.
  $directories_to_make = _tailvite_get_directories_to_make();
  $directories_to_make_status = _tailvite_make_directories($directories_to_make, $theme_path);

  if ($directories_to_make_status !== TRUE) {
    return _tailvite_notify_fail('', 'Failed on Phase: Make directories.');
  }

  // Phase: Copy files.
  $files_to_copy = _tailvite_get_files_to_copy();
  $files_to_copy_status = _tailvite_copy_files($files_to_copy, $theme_path);

  if ($files_to_copy_status !== TRUE) {
    return _tailvite_notify_fail('', 'Failed on Phase: Copy files.');
  }

  // Phase: Alter files.
  $alterations = _tailvite_get_alterations($human_readable_name, $machine_name, $description);
  $files_to_alter = _tailvite_get_files_to_alter();
  $files_to_alter_status = _tailvite_alter_files($theme_path, $files_to_alter, $alterations);
  ...
  if ($files_to_rename_status !== TRUE) {
    return _tailvite_notify_fail('', 'Failed on Phase: Rename files.');
  }

  // Phase 7: Return success message to the user.
  return _tailvite_notify_success($human_readable_name, $theme_path);
}
 ...
 */
function _tailvite_get_alterations($human_readable_name, $machine_name, $description) {
  return array(
    'tailvite' => $human_readable_name,
    'tailvite' => $machine_name,
    'Theme using Vite and component-driven development' => $description,
    'hidden: true' => '',
  );
}
 ...
 */
function _tailvite_get_files_to_alter() {
 ...
 */
function _tailvite_get_directories_to_make() {
  // If we would like to have a bare copy we use is slim option.
  if (drush_get_option('slim') === TRUE) {
    return array(
      'components',
 ...
 */
function _tailvite_get_files_to_copy() {
  // Slim files and directories declaration.
  $default_array = array(
    '.browserslistrc',
    '.eslintignore',
 ...
 */
function _tailvite_get_files_to_rename() {
  // Slim files and directories declaration.
  $default_array = array(
    'tailvite.info.yml',
    'tailvite.theme',
 ...
 *   A boolean representing the success or failure of the function.
 */
function _tailvite_alter_files($theme_path, array $files_to_alter = array(), array $alterations = array(), $absolute = FALSE, int $depth = 0) {
  ...
  foreach ($files_to_alter as $file_to_replace) {
    ...
    if ($file_type === 'dir') {
      ...
      foreach ($files as $file) {
        ...
        $alter_status = _tailvite_alter_files($theme_path, $processed_file, $alterations, TRUE, $depth + 1);
    ...
    elseif ($file_type === 'file') {
      $string_replace_status = _tailvite_file_str_replace($file_path, array_keys($alterations), $alterations);
 ...
 *   A boolean representing the success or failure of the function.
 */
function _tailvite_make_directories(array $directories = array(), string $destination_path = '') {

  // Check for invalid settings and return an error.
  if (empty($destination_path)) {
    _tailvite_notify_fail('', "Invalid parameter passed to _tailvite_make_directories().");
    return FALSE;
  }
  ...
  foreach ($directories as $directory_to_make) {
    ...
    if (!_tailvite_validate_path($directory_path)) {
      return FALSE;
    }
  }
 ...
 *   A boolean representing the success or failure of the function.
 */
function _tailvite_copy_files(array $files = array(), string $destination_path = '') {

  // Check for invalid settings and return an error.
  if (empty($destination_path)) {
    return _tailvite_notify_fail('', "Invalid parameter passed to _tailvite_copy_files().");
  }
 ...
 */
function _tailvite_rename_files($theme_path, $machine_name, array $files_to_rename = array()) {
  foreach ($files_to_rename as $file_to_rename_path) {
    $file_original_path = $theme_path . DIRECTORY_SEPARATOR . $file_to_rename_path;
    $file_new_path = $theme_path . DIRECTORY_SEPARATOR . str_replace('tailvite', $machine_name, $file_to_rename_path);
 ...
 */
function _tailvite_file_str_replace($file_path, array $find, array $replace) {
 ...
 */
function _tailvite_validate_path($path) {
 ...
 */
function _tailvite_validate_path_is_empty($path) {
 ...
 */
function _tailvite_notify_fail($path = '', $message = '') {
